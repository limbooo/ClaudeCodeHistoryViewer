...
- **全文搜索**: 搜索所有对话和消息内容
- **范围选择**: 搜索当前文件或所有项目文件
- **灵活选项**: 大小写敏感搜索、正则表达式支持、文件名搜索
- **丰富结果显示**: 显示文件信息、修改日期和项目文件夹
- **智能导航**: 一键导航到搜索结果，自动展开文件树
- **交互界面**: 左右分栏布局，搜索控制和结果展示
- **实时高亮**: 匹配内容和选中文件的视觉反馈
...
4. **搜索对话**: 使用搜索按钮（🔍）搜索所有对话：
   - 点击搜索按钮打开搜索模态框
   - 输入搜索词并配置选项（大小写敏感、正则表达式、文件名搜索）
   - 选择搜索范围：当前文件或所有文件（默认：所有文件）
   - 点击搜索结果项选中，然后点击"打开文件并跳转到匹配"进行导航
   - 左侧边栏自动展开显示目标文件
   - 右侧面板跳转到精确消息并高亮显示
